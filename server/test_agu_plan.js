...
assert(loadSeries.avgMinutes === 140, `Média dos dias ativos é 140 min, veio ${loadSeries.avgMinutes}`);
assert(loadSeries.homeostasisMinMinutes === 112, `Piso −20% deveria ser 112 min, veio ${loadSeries.homeostasisMinMinutes}`);
assert(loadSeries.homeostasisMaxMinutes === 168, `Teto +20% deveria ser 168 min, veio ${loadSeries.homeostasisMaxMinutes}`);
assert(mondayPoint.zone === 'homeostasis', '140 min está dentro da média dos dias ativos');
...
assert(formatAguHomeostasisStudyVictoryTitle(112) === 'Estudar no mínimo 112 minutos.', 'Título usa o piso da faixa em minutos');
const homeostasisVictory = buildAguHomeostasisStudyVictory(plan, [...exams, laterExam], monday, { days: 14 });
assert(homeostasisVictory.title === 'Estudar no mínimo 112 minutos.', `Vitória usa o piso da faixa, veio ${homeostasisVictory.title}`);
